...
## MAIN ##
def main()->None:
    print(f"\tIdVd PLOTTER\n"
          f"\nParametri di funzionamento principali: "
          f"\n\t·background color (default {cfg.bk_color})"
          f"\n\t·grafici a diversi Vgf nella stessa figura (default {cfg.same_fig_choice})"
          f"\n\t·stessa scala verticale nei grafici di output (default {cfg.same_y_scale})"
    ...
    while True:
        match input("\t"):
            case "yes":
                ...
                while True:
                    choice = input(f"\tsame_fig: {cfg.same_fig_choice} (yes,no,both)-> ")
                    if choice in ("yes", "no", "both"):
                        cfg.same_fig_choice = choice
                        break
                    elif choice == "": break
            ...
            case "no":
                return None # esci dal programma
