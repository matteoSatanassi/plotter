...
class ExpCurves:
    def __init__(self, path:Path):
        self.path:Path = path
        self.td = Curve()
        self.ctd1 = Curve(0.5000)
        self.ctd7 = Curve(0.6160)
        self.ctd8 = Curve(0.7660)
        self.ctd9 = Curve(0.7830)
        self.ctd2 = Curve(0.9670)
        self.ctd3 = Curve(0.9840)
        self.ctd4 = Curve(1.1840)
        self.ctd5 = Curve(1.3340)
        self.ctd6 = Curve(1.8340)
    def sort(self)->None:
    ...
    def data_extraction(self)->None:
        ...
        try:
            ...
            for col in data.columns:
                ...
                for name,curve in vars(self).items():
                    if type(curve) is Curve:
                        if curve.pos is None and "trap_density" in col: right_curve = curve
                        elif str(curve.pos) in col : right_curve = curve
                if "X" in col: right_curve.X = data[col].to_numpy(dtype=float)
                elif "Y" in col: right_curve.Y = data[col].to_numpy(dtype=float)
            self.sort()
